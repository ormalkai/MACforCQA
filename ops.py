...
def inter2att_mask(interactions, dim, snippets_length, dropout = 1.0, name = "", reuse = None):
    """
    ...
    with tf.variable_scope("inter2att" + name, reuse = reuse):
        logits = inter2logits(interactions, dim, dropout = dropout)
        # add mask on the snippets
        # reshape logits to matrix
        snippets_length_shape = tf.shape(snippets_length)
        logits_shape = tf.shape(logits)
        num_snippets = snippets_length_shape[1]
        logits = tf.reshape(logits, shape=(-1, num_snippets, logits_shape[1]//num_snippets))
        logits_mask = expMask(logits, snippets_length)
        # reshape matrix to vector
        logits_mask = tf.reshape(logits_mask, shape=logits_shape)
        attention = tf.nn.softmax(logits_mask)
    return attention
...
def att2Smry_snippet_level_att(attention, features, snippets_length):
    if config.snippet_level_attention:
        print("using snippet level attention")
        # reshape attention to matrix
        snippets_length_shape = tf.shape(snippets_length)
        attention_shape = tf.shape(attention)
        num_snippets = snippets_length_shape[1]
        attention = tf.reshape(attention, shape=(-1, num_snippets, attention_shape[1] // num_snippets))

        # sum of dist per snippet
        attention_per_snippet = tf.reduce_sum(attention, axis=2, keepdims=True)

        # multiply snippet weight in every "word weight" in the snippet
        attention = attention * attention_per_snippet

        # reshape matrix to vector
        attention_mask = expMask(attention, snippets_length)
        attention_mask = tf.reshape(attention_mask, shape=attention_shape)
        # normalize weights to distribution
        attention = tf.nn.softmax(attention_mask)
