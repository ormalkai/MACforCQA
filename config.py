...
def parseArgs():
    ...
    parser.add_argument("--save_weights", action="store_true", help="save graph weights or not ")
    ...
    parser.add_argument("--visualize_graph",     action = "store_true",     help = "save data for graph visualization")
    ...
    # data files
    parser.add_argument("--dataset",         default = "CLEVR", choices = ["CLEVR", "NLVR", "ComplexWebQuestions"], type = str) #
    ...
    parser.add_argument("--snippetDropout", default=0.92,   type = float,    help = "dropout on the snippets vector")
    ...
    ################ snippets input unit (the KB)
    parser.add_argument("--snippetEncDim",          default=512, type=int, help="dimension of snippets encoder RNN")
    ...
    parser.add_argument("--baseline", default=False, type=bool, help="run baseline without the mac network ")
    parser.add_argument("--max_possible_answers",   default = 17, type=int,          help="out dim of classifer last layer, basically this is the length of the snippets")
    # parser.add_argument("--max_possible_answers", default=2547, type=int, help="out dim of classifer last layer, basically this is the length of the snippets")
    parser.add_argument("--top_k_candidates", default=100, type=int,
                        help="keep top k candidates based on tf")
    parser.add_argument("--manual_cross_entropy",   action = "store_true",          help = "compute manual cross entropy")
    parser.add_argument("--sigmoid_cross_entropy",  action="store_true", help="compute sigmoid cross entropy")
    ...
    parser.add_argument("--outSnippet",             action = "store_true",          help="feed the snippet to the output unit")
    parser.add_argument("--outFinalCtrl",           action = "store_true",          help="feed the final control to the output unit")
    ...
    parser.add_argument("--outSnippetMul",          action = "store_true",          help = "feed the multiplication of snippet and memory to the output unit")
    parser.add_argument("--out_contextual_words", action="store_true", help="push contextual words to output unit")
    # mask output
    parser.add_argument("--mask_output_by_padding", action="store_true", help="mask padding only in output unit")
    parser.add_argument("--mask_output_by_pos", action="store_true", help="mask padding and relevant pos")
    parser.add_argument("--mask_output_by_tf_and_stop_words", action="store_true", help="mask padding stop words and choose top k candidates by tf, see also top_k_candidates")
    parser.add_argument("--mask_output_by_ner", action="store_true", help="mask padding and relevant ner")


    ################ network
    parser.add_argument("--netLength", default=5, type=int, help="network length (number of cells)")
    ...
    parser.add_argument("--num_snippets", default=50, type=int, help="number of snippets to use")
    parser.add_argument("--snippets_max_len", default=100, type=int, help="maximum length of every snippet")
    ...
    # snippet level attention
    parser.add_argument("--snippet_level_attention", action="store_true", help="use snippet level attention")
